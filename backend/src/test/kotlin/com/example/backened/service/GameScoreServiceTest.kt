...
class GameScoreServiceTest {
    ...
    fun setup() {
        ...
        gameScoreService = GameScoreServiceImpl(gameScoreRepository,gameModeRepository,gameLevelRepository)
    }
    ...
    fun `getScoreを実行するとJPAGameScoreRepositoryを呼びScoreのListで帰ってきたデータを加工している`() {
        ...
        val score1 = Score(gameScore = 100, user = User(name = "Masaru"), gameMode = modeIrasutoya, gameLevel = gameLevel1)
        val score2 = Score(gameScore = 50, user = User(name = "Masaru2"), gameMode = modePokemon, gameLevel = gameLevel2)
        val score3 = Score(gameScore = 50, user = User(name = "Masaru3"), gameMode = modePokemon, gameLevel = gameLevel1)

        val saveGameScoreResult = gameScoreRepository.saveAll(listOf(score1, score2,score3))
    ...
    fun`postScoreを実行するとJPAGameScoreRepositoryを呼びデータが保存される`() {
        ...
        val saveAndExpectedData= RequestScore(user = "masaru", gameScore = 300, gameMode = "irasutoya", gameLevel = "12")
        ...
        assertEquals(saveAndExpectedData.gameLevel.toInt(), actualResult.gameLevel.level)
    }
